...
| `automation_only_source_patterns` | Source patterns (pipe-separated) accepted only on PRs opened by automation, e.g. `backmerge/*`. Restricts rather than grants — the pattern must still be allowed by `allowed_source_branches` or `source_branch_rules` | string | `''` |
| `automation_source_actors` | Comma-separated logins counting as automation. Empty accepts any author of type `Bot` | string | `''` |
