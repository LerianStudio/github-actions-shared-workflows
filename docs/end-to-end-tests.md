...
| `service_domain` | string | no | `fuzzing.lerian.net` | Base domain; URLs are `https://<app>.<base_env>-<tenancy>.<service_domain>`. Defaults to the fuzzing environment; override to target another. |
