...
| `fail-on-findings` | Fail the step on blocking alerts. When `false`, runs with `--disable-blocking` | No | `false` |
...
| `dry-run` | Print the resolved configuration, force `--disable-blocking` and never fail | No | `false` |
...
| `sarif-file` | Path to the generated SARIF report, empty when not requested |
...
When `fail-on-findings` is `false` (the default) the CLI is invoked with `--disable-blocking`, which makes it always exit `0`. Enforcement is therefore opt-in per repository — the same discipline as `fail_on_coverage_threshold` and the pre-release gate.

## SARIF

Setting `sarif-file` writes a SARIF report and exposes its path as an output. The action does **not** upload it to the GitHub Security tab: that requires Code Security (GHAS) on the repository, the same reason `codeql_upload_sarif` defaults to `false` in `pr-security-scan`. Consuming workflows that do have GHAS can pipe the output into `github/codeql-action/upload-sarif`.
